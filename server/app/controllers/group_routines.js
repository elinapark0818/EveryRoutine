...
module.exports = {
  ...
  join_group_routine: {
    get: async (req, res) => {
      ...
      // 그룹 - 유저간 데이터 추가하기
      await group_user.create({ user_id: myUserId, group_routine_id: groupRoutineID });

      return res.status(200).json({ data: null, message: "그룹 가입에 성공하였습니다" });
      }
  ...
  write_comment: {
    post: async (req, res) => {
      ...
      return res.status(201).json({ data: { comment : selectedCommentArray }, message: "댓글이 추가되었습니다" })
    },
  },
    ...
    get: async (req, res) => {
      ...
      try {
        ...
        const makeCommentArray = async () => {
          for(let el of selectedComment) {
            const name = await findUserName(el.user_id)
            selectedCommentArray.push({ writer: name, comment: el.comment, time: `${el.createdAt.getHours()}:${el.createdAt.getMinutes()}`});
          }
        }
        await makeCommentArray();
  ...
  withdrawal_group_routine: {
    get: async (req, res) => {
      ...
      try {
        ...
        // 탈퇴 전 우선 댓글 삭제하기
        await comment.destroy({ where: { user_id: myUserId, group_routine_id: groupRoutineID }});

        // 댓글 삭제 후 그룹-유저 관계 삭제하기
        await group_user.destroy({ where: { user_id: myUserId, group_routine_id: groupRoutineID }});

        return res.status(200).json({ data: null, message: "그룹을 탈퇴하였습니다" });
      }
  ...
  delete_group_routine: {
    get: async (req, res) => {
      ...
      try {
        ...
        const deleteRoutine = await group_routine.findOne({ where: { id: groupRoutineID }})
        ...
        if(!imEditor) {
          return res.status(203).json({ data: null, message: "삭제할 권한이 없습니다"});
        } else {
          if(!deleteRoutine) {
            return res.status(203).json({ data: null, message: "이미 삭제된 그룹루틴입니다"});
          } else {
            // 해당 댓글 삭제하기
